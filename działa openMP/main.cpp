...
long int gaussian_value(long int* img, int width, int y, int x, int mask[], int mask_size, int half_mask){
    ...
    for(y1 = 0; y1 < mask_size; y1++){
        for(x1 = 0; x1 < mask_size; x1++){
            // printf("(tid=%d) ms:%d x1: %d, y1: %d\n",omp_get_thread_num(), mask_size, x1, y1); 
...
bool* threshold_response(long long* r_arr, int width, int height, long long threshold, int n_dim, int max_corners){ // 1 if value above threshold 0 if below for each pixel
    ...
    // #pragma omp parallel for default(shared) collapse(2) private(y1,x1,x,y,r)
    // for (y = 0; y < height; y++){
    //     for (x=0; x < width; x++){
    //         r = r_arr[y*width + x];
    //         if (threshold < r){
    //             // printf("a ");
    //             bool not_max_in_neigh = false;
    //             for (y1 = -n_dim; y1 <= n_dim; y1++){
    //                 for (x1 = -n_dim; x1 <= n_dim; x1++){
    //                     if ((y + y1)*width + (x + x1) >=0 && (y + y1)*width + (x + x1) < arr_size && // neigbor inside image
    //                     r_arr[(y + y1)*width + (x + x1)] > r){ // neighbor has higher value
    //                         not_max_in_neigh = true;
    //                         break;
